...
def add_current_limits(
    ...
    """
    _validate_config(permanent_margin, temporary_tiers)
    ...
    for type_name, getter_name, sides, updater, flag in _BRANCH_TYPES:
        ...
        for eid, elem in df.iterrows():
            ...
            for current_col, side, group_col in sides:
                current = elem[current_col]
                if not _is_usable(current, min_current):
                    type_stats["skipped_no_flow"] += 1
                    stats["skipped_no_flow"] += 1
                    continue
                rows.extend(_limit_rows(eid, side, float(current),
...
def _limit_rows(element_id: str, side: str, current: float,
                permanent_margin: float,
                temporary_tiers: Sequence[Tuple[int, float]],
                group_name: str) -> List[dict]:
    """One permanent + N temporary CURRENT limits for a single branch side."""
    permanent = current * permanent_margin
    rows = [{
        "element_id": element_id, "side": side, "name": "permanent_limit",
        "type": "CURRENT", "value": permanent,
    ...
    for duration, margin in temporary_tiers:
        rows.append({
            ...
            "name": _tier_name(duration), "type": "CURRENT",
...
def loading_report(network: pn.Network, group_name: str = DEFAULT_GROUP_NAME,
    ...
    (current / permanent limit) and how many sides exceed their permanent
    limit - which, by construction, should be none in the base case.
    """
    if run_loadflow:
        _run_ac_or_raise(network, lf_parameters)
    ...
    is_current = limits.index.get_level_values("type") == "CURRENT"
    is_permanent = limits.index.get_level_values("acceptable_duration") == PERMANENT_DURATION
    perm = limits[is_current & is_permanent]

    currents = _side_currents(network)
    checked = 0
    max_loading = 0.0
    overloaded = 0
    for (element_id, side, _type, _dur, grp), row in perm.iterrows():
        if group_name is not None and grp != group_name:
            continue
        current = currents.get((element_id, side))
        value = row["value"]
        if current is None or not math.isfinite(current) or not value:
            continue
...
def _side_currents(network: pn.Network) -> Dict[Tuple[str, str], float]:
    ...
    for _type_name, getter_name, sides, _updater, _flag in _BRANCH_TYPES:
        ...
        if df.empty:
            continue
        for eid, elem in df.iterrows():
            for current_col, side, _ in sides:
                out[(eid, side)] = elem[current_col]
    return out
