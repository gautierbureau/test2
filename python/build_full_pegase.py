...
load-flow-based current + apparent-power limit sets (one group per season) - and,
by default, rebuilds the result as a node-breaker network (busbar sections +
switches, with element properties carried across) so it looks like a real one.
...
# Seasonal limit ratings: one operational-limit group per season, scaled from
# the base rating (higher in cold weather). Real networks carry per-season
# limits; our IIDM schema represents them as named groups (newer powsybl surfaces
# the same data as the currentLimitsPerSeason extension).
SEASONS = {"WINTER": 1.1, "SPRING": 1.0, "SUMMER": 0.9, "AUTUMN": 1.0}
...
def build_full(input_path: str, output_path: str,
    ...
    steps = [
        ...
        ("current + apparent limits",
         lambda: acl.add_current_limits(
             net, run_loadflow=False, limit_types=("CURRENT", "APPARENT_POWER"),
             seasons=SEASONS, selected_season="WINTER")),
    ]
